  ...
  return (
    <>
      <div className="col-span-2 print:hidden sm:col-span-3 lg:col-span-2 ">
        ...
        <div className="flex py-8 min-h-full sm:border-r md:pe-4 flex-col gap-2 max-w-[280px] mx-auto  dark:border-gray-700">
          ...
          {menuItemsData.map((menu, index) => {
            ...
            const active_class = is_open === true ? ' border-s-4 font-bold text-black bg-slate-200 dark:bg-slate-800  rounded-md' : 'rounded-md';
              ...
              <Menudropdown key={index} is_open={is_open} has_submenu={has_subm}  >
                <Menudropdown.Trigger>
                  {(menu.url && menu.route != '') ?
                    ...
                    <Link href={menu.url}
                      className={"inline-flex  items-center h-5w-5 text-start md:w-full px-2 py-1 ms-2 text-md mb-0 leading-4 font-medium text-gray-900_ dark:text-gray-100   hover:text-gray-700 dark:hover:text-gray-300 dark:hover:bg-slate-700 focus:outline-none transition ease-in-out duration-150 " + active_class}

                    >{menu.icon} <span className='hidden sm:flex'><Translate>{menu.title}</Translate></span>
                    </Link>
                    :
                    <button type="button"
                      className={"items-center md:w-full  inline-flex text-start px-2 py-1 ms-2 text-md mb-0 leading-4 font-medium  text-gray-900_ dark:text-gray-100   hover:text-gray-700 dark:hover:text-gray-300 dark:hover:bg-slate-700 focus:outline-none transition ease-in-out duration-150 " + active_class}
                  ...
                  {menu.sub && menu.sub.length &&
                    <Menudropdown.Content >
                      {menu.sub.map((smenu, sindex) => {
                        const smactive_class = (page_subid === smenu?.sid) ? ' font-bold text-blue-600 dark:text-yellow-500 ' : '';
