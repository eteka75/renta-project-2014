...
export default function Pagination({ links,className }) {
    ...
    function getClassName(active) {
        if (active) {
            ...
            return "mr-1 mb-1 px-4 py-3 text-sm bg-white leading-4 border text-sm rounded hover:bg-white hover:text-gray-900 focus:border-primary focus:text-primary";
        }
    }
                    ...
                    {links && Array.isArray(links) && links.map((link, key) => (
                        link.url === null ?
                            (<div key={key+'sh'+Math.random(1)+Math.random(1)*+Math.random(1)}
                                className="mr-1 mb-1 px-4 py-3 text-sm leading-4 bg-white text-gray-400 border rounded"
