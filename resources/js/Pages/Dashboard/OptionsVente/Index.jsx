...
export default function Index({ auth, option_ventes, page_id, 
    page_subid, page_title, page_subtitle, search_text = '',count }) {

    const TABLE_HEAD = ["Photo", "Nom",'Prix',  "Date d'ajout", "Actions"];
    ...
    useEffect(() => {
        if (option_ventes.data && option_ventes.data.length > 0) {
            setDatas(option_ventes.data);
        }
    ...
    const SubmitDeletion = () => {
        if (setDeleteId != '') {
            Inertia.delete(route('dashboard.option_ventes.delete', deleteId));
            setDeleteId('')
            setSupDialog(false);
        } else {
            ReloadPage();
    ...
    const Search = (e) => {
        e.preventDefault();
        if (data.search !== '') {
            get(route('dashboard.option_ventes.search'),
                {
                    onSuccess: (response) => {
        ...
        <DashboardLayout auth={auth} page_id={page_id} page_subid={page_subid}>
            ...
            <DashHeadTitle title={page_title} subtitle={page_subtitle} >
                <Link className='inline-flex whitespace-nowrap items-center text-sm sm:text-md px-5 py-2 text-white bg-blue-600 hover:bg-blue-700 focus:bg-blue-700 rounded-md md:ml-6 md:mb-3'
                    href={route('dashboard.option_ventes.create')}>
            ...
            <Card className="h-full w-full dark:bg-gray-800/30 dark:border-slate-800 dark:border dark:text-white">
                <SearchBar
                    exportUrl={route('dashboard.option_ventes.export')}
                ...
                <CardBody className={" p-0 overflow-auto"}>
                    <ViewTable count={count}  head={TABLE_HEAD} links={option_ventes.links} showHead={showHead}>
                        {datas.length > 0 && datas.map(
                            ({ id, nom,  photo,prix, created_at }, index) => {
                                    ...
                                    {photo!='' && photo!=null &&
                                        <td className={classes}>
                                            <div className="flex items-center gap-3">
                                                ...
                                                {<Link href={route('dashboard.option_ventes.show', id)}><Avatar src={HTTP_FRONTEND_HOME + '' + photo} alt={nom} className='w-10 bg-white' size="sm" /></Link>}
                                        ...
                                        <td colSpan={ photo!='' && photo!=null?'':'2'} className={classes}>
                                            <div className="flex flex-col">
                                                <Typography
                                                    ...
                                                    className="font-bold"
                                                >
                                                    <Link href={route('dashboard.option_ventes.show', id)}>
                                                        {nom}
                                                    </Link>
                                                </Typography>
                                            </div>
                                        ...
                                        <td className={classes}>
                                            ...
                                            >
                                                ...
                                                {formaterMontant(prix,i18n.language)??''}
                                        ...
                                        <td className={classes}>
                                            <div className="md:flex grid-cols-1 grid md:grid-cols-3 gap-1">
                                                <IconButton title='Modifier' variant="text" className=' text-blue-500'>
                                                    <Link className='flex gap-1 cursor-pointer items-center' href={route('dashboard.option_ventes.edit', id)}>
                                                ...
                                                <IconButton title='Voir' variant="text" className=' text-blue-500'>
                                                    <Link className='flex gap-1 cursor-pointer items-center' href={route('dashboard.option_ventes.show', id)}>
                                                ...
                                                <IconButton variant='text' className='text-red-600 items-center flex gap-1' title="supprimer cet enrégistrement"
                                                    method="delete"
                                                    href={route('dashboard.option_ventes.delete', id)}
                                                    as="button"
                        ...
                        {(datas.length === 0 || (data.search != null && search_text != null)) &&
                            <tr><td className="p-4 border-t border-blue-gray-50 dark:border-slate-800" colSpan={TABLE_HEAD.length}>
                                <div className='text-center text-gray-600 py-10'>
                                    ...
                                    {(data.search != null && search_text != null) && <Link href={route('dashboard.option_ventes')}>
