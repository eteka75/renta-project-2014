...
export default function CarburantForm({ className = '', carburant = null, pays = [], action, btntext = 'Enrégister' }) {
    ...
    const { data, setData, post, put, progress, errors, processing, recentlySuccessful } = useForm(carburant !== null && action === 'update' ?
        {
            nom: carburant.nom ?? '',
            photo: '',
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (action === 'update') {
            post(route('dashboard.carburants.update', carburant.id), data, {
                onSuccess: () => {
        ...
        if (action === 'save') {
            post(route('dashboard.carburants.store'), {
                onSuccess: () => {
                    // Handle success, e.g., redirect
                    //alert('Ok')
                },
