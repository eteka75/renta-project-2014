...
export default function Index({ auth, locations, page_id, count, page_subid, page_title, page_subtitle, search_text = '' }) {
    ...
    const SubmitDeletion = () => {
        if (setDeleteId != '') {
            Inertia.delete(route('dashboard.locations.delete', deleteId));
            setDeleteId('')
            setSupDialog(false);
        } else {
            ReloadPage();
    ...
    const Search = (e) => {
        e.preventDefault();
        if (data.search !== '') {
            get(route('dashboard.locations.search'),
                {
                    onSuccess: (response) => {
    ...
    return (
        <DashboardLayout auth={auth} page_id={page_id} page_subid={page_subid}>
            ...
            <Card className="h-full w-full dark:bg-gray-800/30 dark:border-slate-800 dark:border dark:text-white">
                <SearchBar
                    exportUrl={route('dashboard.locations.export')}
                                        ...
                                        <td className={classes}>
                                            <div className="md:flex grid-cols-1 grid md:grid-cols-3 gap-1">
                                                <IconButton title='Modifier' variant="text" className=' text-blue-500'>
                                                    <Link className='flex gap-1 cursor-pointer items-center' href={route('dashboard.locations.edit', id)}>
                                                ...
                                                <IconButton title='Voir' variant="text" className=' text-blue-500'>
                                                    <Link className='flex gap-1 cursor-pointer items-center' href={route('dashboard.locations.show', id)}>
                                                ...
                                                <IconButton variant='text' className='text-red-600 items-center flex gap-1' title="supprimer cet enrégistrement"
                                                    method="delete"
                                                    href={route('dashboard.locations.delete', id)}
                                                    as="button"
                        ...
                        {(datas.length === 0 || (data.search != null && search_text != null)) &&
                            <tr><td className="p-4 border-t border-blue-gray-50 dark:border-slate-800" colSpan={TABLE_HEAD.length}>
                                <div className='text-center text-gray-600 py-10'>
                                    ...
                                    {(data.search != null && search_text != null) && <Link href={route('dashboard.locations')}>
