...
import { LocaVoitureCard, MiniCard, VenteVoitureCard } from '@/components/locations/LocaVoitureCard'
...
export default function Marque({ marque, locations , marques}) {
  return (
    <FrontLayout>
        ...
        {locations?.data != null && locations?.data?.length > 0 &&
                <>
                  <div className="md:grid sm:grid-cols-2 lg:grid-cols-3 md:mt-4  md:gap-4">
                    {locations?.data != null && locations?.data?.length > 0 && locations?.data?.map(({ voiture, id, tarif_location_heure,
                      tarif_location_journalier, tarif_location_hebdomadaire,
                      tarif_location_mensuel, duree_garantie, kilometrage, prix_vente
                    }, index) => {
                      return <LocaVoitureCard
                        id={id}
                  ...
                  <div className="mb-4">
                    <Pagination links={locations?.data?.links} />
                  </div>
                </>
              }
        </div>
